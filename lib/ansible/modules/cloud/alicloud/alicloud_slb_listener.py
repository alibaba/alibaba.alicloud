...
options:
  ...
  state:
    ...
    required: true
    default: 'present'
    choices: [ 'present', 'absent', 'list', start, stop]
  listener_type:
  ...
  x_forwarded_for:
    description:
        - Whether to start to obtain the actual IP address of a visitor by means of X-Forwarde d-For.
    default: on
    required: false
    choices: ['on', 'off']
  scheduler:
  ...
  health_check_http_code:
    description:
        - Regular health check HTTP status code. Multiple codes are segmented by 鈥�,鈥�. Required when C(health_check='on').
    default: http_2xx
    required: false
    type: string
  ...
  source_items:
    description:
        -Access control list, required when add or remove listener white list item.
    required: false
    type: string
"""
...
def get_info(obj):
    ...
    if hasattr(obj, 'listener_port'):
        result['listener_port'] = obj.listener_port
    if hasattr(obj, 'backend_server_port'):
    ...
    if hasattr(obj, 'x_forwarded_for'):
        result['x_forwarded_for'] = obj.x_forwarded_for
...
def main():
    ...
    argument_spec.update(dict(
        ...
        state=dict(type='str', required=True, choices=['present', 'absent', 'list', 'start', 'stop']),
        ...
        x_forwarded_for=dict(type='str', required=False, default='on', choices=['on', 'off']),
    ...
    x_forwarded_for = module.params['x_forwarded_for']
    ...
    v_server_group_id = module.params['v_server_group_id']
    gzip = module.params['gzip']
    server_certificate_id = module.params['server_certificate_id']
    v_server_groupId = module.params['v_server_groupId']
    ...
    listener = assemble_request_data(listener, 'x_forwarded_for', x_forwarded_for)
    ...
    listener = assemble_request_data(listener, 'health_check_url', health_check_url)
    ...
    listener = assemble_request_data(listener, 'v_server_group_id', v_server_group_id)
    ...
    listener = assemble_request_data(listener, 'v_server_groupId', v_server_groupId)
    ...
    if state == "present":
        if backend_server_port:
            ...
            changed = slb.set_listener_access_control_status(listener)
        elif source_items:
            #add listener_white_list_item
            changed = slb.add_listener_white_list_item(listener)
        else:
            #set attribute
            changed = slb.set_listener_attribute(listener)
        module.exit_json(changed=changed, load_balancer_id = load_balancer_id)
    if state == "absent":
        if source_items:
            #remove listener_white_list_item
            changed = slb.remove_listener_white_list_item(listener)
            module.exit_json(changed=changed, load_balancer_id=load_balancer_id)
        #delete 
        changed = slb.delete_load_balance_listener(listener)
        module.exit_json(changed=changed, load_balancer_id=load_balancer_id)
    if state == "start":
        #start
        changed = slb.start_load_balance_listener(listener)
        module.exit_json(changed=changed, load_balancer_id=load_balancer_id)
    if state == "stop":
        #stop
        changed = slb.stop_load_balance_listener(listener)
        module.exit_json(changed=changed, load_balancer_id=load_balancer_id)
    if state == 'list':
        #list attribute
        obj = slb.list_load_balance_listener_attribute(listener)
