...
options:
  ...
  vserver_group_id:
    description:
        - VVirtual server group ID, when the VserverGroup is on, the incoming VServerGroupId value takes effect.
    type: string
  gzip:
    description:
...
listener_port:
    description:
        - Load Balancing Instances Front end ports used.
    returned: on absent
    type: int
    sample: 80
listener_type:
...
def main():
    ...
    if state == "absent":
        ...
        module.exit_json(changed=changed, load_balancer_id=load_balancer_id, listener_port=current_listener.listener_port, listener_type=listener_type)
    if state == "running":
        #start
    ...
    if state == 'list':
        #list attribute
        module.exit_json(changed=True, load_balancer_id=load_balancer_id, listener=get_info(current_listener))
