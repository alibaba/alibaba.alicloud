  ...
  backend_servers:
    description:
        ...
        - '[{"key":"value", "key":"value"}]', keys allowed:
            - server_id (required:true; default: null, type: string) - The server id is the ECS instance Id.
...
- name: set VServer Group attribute in SLB
  ...
  vars:
    ...
    vserver_group_name: test
    backend_servers:
       -  server_id: <your-specified-ECS-instance-id>
          port: 8080
          weight: 100
  tasks:
    - name: Set VServer Group attribute in SLB
...
- name: Add VServer Group Backend Servers in SLB
  ...
  vars:
    ...
    vserver_group_id: <your-specified-vserver-group-id>
    backend_servers:
       -  server_d: <your-specified-ECS-instance-id>
          port: 8080
          weight: 100
  tasks:
    - name: Add VServer Group Backend Servers in SLB
    ...
    if state == "present":
        if current_vserver_group:
            if vserver_group_name:
                obj = current_vserver_group.set_attribute(vserver_group_name=vserver_group_name, backend_servers=backend_servers)
                changed = True
                module.exit_json(changed=changed, vserver_group=get_info(current_vserver_group.describe_attribute()))
            elif backend_servers:
                set_data = []
                add_data = []
                decode_info = byteify(current_vserver_group.backend_servers)
                for server in backend_servers:
                    for item in decode_info['backend_server']:
                        if server['port'] == item['port']:
                            if server['weight'] != item['weight']:
                                set_data.append(server)
                        else:
                            add_data.append(server)
                if set_data:
                    current_vserver_group.set_attribute(backend_servers=set_data)
                if add_data:
